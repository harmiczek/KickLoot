(function () {
    ...
    function updateBadge(status, val = '', rewardInfo = {}) {
        ...
        let rewardNameStr = rewardInfo.rarity ? `(${rewardInfo.rarity})` : '';

        if (status === 'claimed') {
            text = '✔';
            color = '#53F700'; 
            tooltip = rewardNameStr ? `${EXT_NAME}: Looted - ${rewardNameStr}` : `${EXT_NAME}: Reward Claimed!`;
    ...
    function extractRewardInfo(obj) {
        if (!obj) return { rarity: null, image: null };
        let image = obj.card_url || obj.image_url || null;
        let rarity = obj.rarity || null;
        if (rarity) {
             rarity = rarity.charAt(0).toUpperCase() + rarity.slice(1);
        }
        return { rarity: rarity || null, image: image };
    }
    ...
    function dailyRewardInfo(o) {
        ...
        } catch (e) { return { rarity: null, image: null }; }
    }

    function getSavedRewardInfo() {
        const state = readState(D_STATE);
        return {
            rarity: state.lastRewardRarity || null,
    ...
    async function dailyCycle() {
        ...
        try {
            ...
            if (ch.status === 'claimed') {
                ...
                if (info.rarity || info.image) {
                    writeState(D_STATE, { claimedUntil: reset, lastRewardRarity: info.rarity, lastRewardImage: info.image });
                } else {
            ...
            if (ch.status === 'claimable') {
                ...
                try {
                    ...
                    let winnerInfo = { rarity: null, image: null };
                    try {
                    ...
                    writeState(D_STATE, { claimedUntil: reset, lastRewardRarity: winnerInfo.rarity, lastRewardImage: winnerInfo.image, lastClaimAt: Date.now(), backoffUntil: 0 });
